...
interface MenuService {
  // Menu Management
  getMenu(restaurantId: string, language?: string): Promise<Menu>
  getMenuItem(itemId: string, language?: string): Promise<MenuItem>
  updateItemAvailability(itemId: string, available: boolean): Promise<void>
  
  // QR Code Management
  generateQRCode(restaurantId: string, tableId: string): Promise<QRCode>
  getMenuByQRCode(qrCodeId: string): Promise<Menu>
  
  // Translation
  translateMenu(menu: Menu, targetLanguage: string): Promise<Menu>
}
...
interface AIChatService {
  // Chat Operations
  sendMessage(sessionId: string, message: string, language: string): Promise<ChatResponse>
  createSession(restaurantId: string, customerId?: string): Promise<ChatSession>
  
  // Context Management
  getMenuContext(restaurantId: string): Promise<MenuContext>
}
...
interface OrderService {
  ...
  // Order History
  getOrderHistory(restaurantId: string, filters: OrderFilters): Promise<Order[]>
...
interface ForecastingService {
  // Forecast Generation
  generateDailyForecast(restaurantId: string, date: Date): Promise<DemandForecast>
  getForecast(restaurantId: string, itemId: string, dateRange: DateRange): Promise<ItemForecast[]>
  
  // Model Management
  trainModel(restaurantId: string, historicalData: OrderData[]): Promise<ModelMetrics>
  updateModel(restaurantId: string): Promise<void>
}
...
```typescript
interface WastageService {
  // Wastage Tracking
  recordWastage(entry: WastageEntry): Promise<void>
  getWastageReport(restaurantId: string, period: DateRange): Promise<WastageReport>
  
  // Pattern Analysis
  analyzeWastagePatterns(restaurantId: string): Promise<WastagePattern[]>
  getWastageAlerts(restaurantId: string): Promise<WastageAlert[]>
  
  // Surplus Management
  markSurplusFood(itemId: string, quantity: number, discountPercent: number): Promise<SurplusListing>
  listForDonation(itemId: string, quantity: number, charityId: string): Promise<DonationListing>
  trackSurplusDisposition(surplusId: string, disposition: DispositionType): Promise<void>
}
...
interface WastagePattern {
  ...
  temporalPattern: 'end-of-day' | 'day-of-week' | 'seasonal'
...
interface InventoryService {
  // Stock Management
  updateStock(ingredientId: string, quantity: number, operation: 'add' | 'subtract'): Promise<void>
  getCurrentStock(restaurantId: string): Promise<StockLevel[]>
  
  // Optimization
  generateStockRecommendations(restaurantId: string): Promise<StockRecommendation[]>
  getReorderAlerts(restaurantId: string): Promise<ReorderAlert[]>
}
...
interface NotificationService {
  ...
  scheduleNotification(notification: NotificationRequest, scheduledFor: Date): Promise<string>
  
  // Targeting
  sendToSegment(segment: CustomerSegment, notification: NotificationContent): Promise<NotificationResult>
  sendLocationBased(location: GeoLocation, radius: number, notification: NotificationContent): Promise<NotificationResult>
  
  // Analytics
  getNotificationMetrics(notificationId: string): Promise<NotificationMetrics>
  
  // Preferences
  updateCustomerPreferences(customerId: string, preferences: NotificationPreferences): Promise<void>
}

interface NotificationRequest {
  restaurantId: string
  targetCustomers: string[] | 'all'
  content: NotificationContent
  type: 'marketing' | 'surplus' | 'special_offer'
  scheduledFor?: Date
}
...
interface AnalyticsService {
  // Dashboard Metrics
  getDashboardMetrics(restaurantId: string, dateRange: DateRange): Promise<DashboardMetrics>
  getRealtimeMetrics(restaurantId: string): Promise<RealtimeMetrics>
  
  // Detailed Analytics
  getPeakHoursAnalysis(restaurantId: string, dateRange: DateRange): Promise<PeakHoursData>
  getPopularDishes(restaurantId: string, dateRange: DateRange, limit: number): Promise<PopularDish[]>
  getCustomerBehavior(restaurantId: string): Promise<CustomerBehaviorMetrics>
  
  // Insights
  generateInsights(restaurantId: string): Promise<Insight[]>
}
...
interface DashboardMetrics {
  ...
  topPerformers: PopularDish[]
}

interface PeakHoursData {
  hourlyDistribution: { hour: number; orderCount: number; revenue: number }[]
  peakHours: number[]
  recommendedStaffing: { hour: number; staffCount: number }[]
}
```
...
interface AuthService {
  ...
  // Audit
  logAccess(userId: string, resource: string, action: string, granted: boolean): Promise<void>
}
...
type UserRole = 'owner' | 'manager' | 'chef' | 'customer'

interface RolePermissions {
  owner: string[] // All permissions
  manager: string[] // Analytics, inventory, orders, customers
  chef: string[] // Menu items, orders, wastage
  customer: string[] // View menu, place orders
}
```
...
type WastageReason = 'spoilage' | 'over_preparation' | 'customer_return' | 'cooking_error' | 'quality_issue' | 'other'
...
type SurplusStatus = 'available' | 'sold' | 'donated' | 'expired' | 'wasted'
...
type NotificationType = 'marketing' | 'surplus_food' | 'special_offer' | 'new_menu_item' | 'loyalty_reward'
...
function analyzeWastagePatterns(entries: WastageEntry[]): WastagePattern[] {
  // Group by menu item
  const byItem = groupBy(entries, 'menuItemId')
  
  return Object.entries(byItem).map(([itemId, itemEntries]) => {
    // Calculate average waste rate
    const totalWasted = sum(itemEntries.map(e => e.quantity))
    const totalOrdered = getTotalOrdered(itemId, entries[0].timestamp)
    const wasteRate = totalWasted / totalOrdered
    
    // Identify temporal patterns
    const temporalPattern = identifyTemporalPattern(itemEntries)
    
    // Generate recommendation
    const recommendation = generateRecommendation(wasteRate, temporalPattern)
    
    return { itemId, wasteRate, temporalPattern, recommendation }
  })
}
```
...
```typescript
function generateStockRecommendation(ingredient: Ingredient, forecast: DemandForecast): StockRecommendation {
  // Calculate expected usage based on forecast
  const expectedUsage = calculateExpectedUsage(ingredient, forecast)
  
  // Account for lead time
  const leadTimeUsage = expectedUsage * (ingredient.leadTimeDays / 7)
  
  // Calculate safety stock (buffer for uncertainty)
  const safetyStock = expectedUsage * 0.2
  ...
  const optimalStock = ingredient.optimalStock
  
  if (currentStock <= reorderPoint) {
    const recommendedQuantity = Math.max(
      optimalStock - currentStock,
      expectedUsage * 2 // At least 2 weeks supply
    )
    
    // Adjust for shelf life
    const maxQuantity = ingredient.shelfLifeDays * (expectedUsage / 7)
    const finalQuantity = Math.min(recommendedQuantity, maxQuantity)
    
    return {
      ingredientId: ingredient.id,
      recommendedOrderQuantity: finalQuantity,
      justification: `Current stock (${currentStock}) below reorder point (${reorderPoint})`,
...
async function sendNotification(request: NotificationRequest): Promise<NotificationResult> {
  ...
  const customers = await getTargetCustomers(request)
  const eligibleCustomers = await filterByRateLimit(customers)
  
  // 2. Filter by preferences
  const optedInCustomers = eligibleCustomers.filter(c => 
    c.notificationOptIn && 
    c.preferences.notificationTypes.includes(request.type)
  )
  
  // 3. Get device tokens
  const deviceTokens = await getDeviceTokens(optedInCustomers)
  
  // 4. Send via FCM
  const fcmResponse = await fcm.sendMulticast({
    tokens: deviceTokens,
    notification: {
  ...
  // 5. Track delivery
  await trackDelivery(request.id, fcmResponse)
  
  // 6. Update rate limit counters
  await updateRateLimitCounters(optedInCustomers)
...
const RATE_LIMITS = {
  'POST /api/v1/orders': { points: 10, duration: 60 }, // 10 orders per minute
  'POST /api/v1/chat/*/messages': { points: 10, duration: 60 }, // 10 messages per minute
  'POST /api/v1/auth/login': { points: 5, duration: 300 }, // 5 attempts per 5 minutes
  'POST /api/v1/notifications': { points: 100, duration: 3600 } // 100 notifications per hour
}
...
// Feature: restaurant-intelligence-platform, Property 2: Menu Item Completeness
describe('Menu Item Completeness Property', () => {
  it('should return all required fields for any menu item', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
        ...
        async ({ restaurantId, itemId }) => {
          // Create menu item
          await createMenuItem(restaurantId, itemId)
...
2. Chef records wastage → pattern identified → alert generated
